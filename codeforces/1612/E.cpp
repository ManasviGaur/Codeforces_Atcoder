...
void solve(){
    ...
    int mssgs = 0;
    set<int> s;
    for(pair<int, int> p: a) s.insert(p.first);
    mssgs = s.size();
    for(int i = 1; i <= min((int)20, mssgs); i++){
        map<int, double> prob;
        for(int j = 0; j < n; j++){
            if(a[j].second >= i) prob[a[j].first]++;
            else prob[a[j].first] += (1.0*a[j].second)/i;
        }
        vector<pair<double, int>> tmp;
        for(auto [p, q]: prob) tmp.push_back({q, p});
        sort(rall(tmp));
        vector<int> ans1;
        double prob1 = 0;
        for(int j = 0; j < i; j++){
            prob1 += tmp[j].first;
            ans1.push_back(tmp[j].second);
        }
        debug() << deb(i) deb(ans1) deb(prob1);
        if(prob1 > prb){
            ans = ans1;
            prb = prob1;
        }
