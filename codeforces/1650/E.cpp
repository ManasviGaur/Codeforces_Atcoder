...
int fun(vector<int> a0, int ind, int mu){
    vector<int> a;
    for(int i = 0; i < n + 2; i++){
        if(i == ind) continue;
        a.push_back(a0[i]);
    }
    debug() << deb(a0) deb(ind) deb(a);
    int mu2 = inf, mx = 0;
    for(int i = 1; i < n; i++){
        mu2 = min(mu2, a[i] - a[i-1] - 1);
        mx = max(mx, a[i] - a[i-1] - 1);
    }
    debug() << deb(mu2) deb(mx);
    if(mu2 == mu)
        return mu;
    return min(mu2, max((mx-1)/2, a[n] - a[n-1] - 2));
}
...
void solve(){
    ...
    debug() << deb(ind) deb(mu);
    if(ind == 1){
        cout << fun(a, ind, mu);
    }else{
        cout << max(fun(a, ind, mu), fun(a, ind - 1, mu));
    }
    debug() << deb('\n');
    cout << endl;
}
