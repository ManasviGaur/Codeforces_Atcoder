...
void solve(){
    ...
    sort(all(a));
    disjointSet ds(n);
    set<vector<int>> red, blue; // y, x, ind
    for(int i = 0; i< n ;i++){
        while(!red.empty()){
            vector<int> v = *red.begin();
            if(v[0] < a[i][0]) red.erase(red.begin());
            else break;
        }

        while(!blue.empty()){
            vector<int> v = *blue.begin();
            if(v[0] < a[i][0]) blue.erase(blue.begin());
            else break;
        }

        if(a[i][2] == 0){
            red.insert({a[i][1], a[i][0], i});
            for(vector<int> v: blue){
                ds.dsu(i, v[2]);
            }
            while(blue.size() > 1) blue.erase(blue.begin());
        }else{
            blue.insert({a[i][1], a[i][0], i});
            for(vector<int> v: red){
                ds.dsu(i, v[2]);
            }
            while(red.size() > 1) red.erase(red.begin());
        }
    }
    
    cout << ds.countDisjointSets() << endl;
}
