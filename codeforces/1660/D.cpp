...
void solve(){
    ...
    for(auto [l,r]: v){
        int i = l, j = r;
        int curr1 = 0, curr2 = 0, cnt = 0;
        while(i <= j) curr1 += (abs(a[i]) == 2 ? 1 : 0), cnt += (a[i++] < 0 ? 1 : 0);
        debug() << deb(l) deb(r) deb(curr1);
        if(cnt%2 == 0){
            if(ans < curr1){
                a1 = l-1;
                a2 = n-r;
