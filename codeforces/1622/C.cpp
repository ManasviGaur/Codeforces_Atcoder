...
void solve(){
    ...
    sort(a.begin(), a.end());
    debug() << deb(res) deb(a) deb(sum);
    sum -= a[0];
    for(int i = n - 1; i >= 1; i--){
      sum -= a[i];
      x++;
      int new_num = (k - sum)/(x+1);
      while(sum + (x+1)*new_num > k)
        new_num--;
      new_num = min(new_num, a[0]);
      if(sum + (x+1)*new_num <= k)
        res = min(res, x + (a[0] - new_num));
      debug() << deb(res) deb(sum) deb(new_num) deb(sum + (x+1)*new_num);
    }
    cout << res << endl;
}
