...
const int N = 3e5+5, inf = 1e17, M = 1e9 + 7;
int test, n, m, k, a[N], b[N], vt[N][3];
vector<int> g[N];
...
void dfs(int u, int par = 0) {
	...
	for(auto v : g[u]) if(v != par) {
		...
		if(b[v] == 1) {
			vt[u][1] += vt[v][1];
			vt[u][0] += vt[v][1] + 1;
			vt[u][2] += min(vt[v][1] + 1, vt[v][2]);
		} else {
			vt[u][1] += vt[v][0] + 1;
			vt[u][0] += vt[v][0];
			vt[u][2] += min(vt[v][0] + 1, vt[v][2]);
		}
	}
}
...
void solve() {
	...
	dfs(1);
	cout << min(vt[1][b[1]] + 1, vt[1][2]) << endl;
}
